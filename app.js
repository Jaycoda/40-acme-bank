...
app.use((error, request, response, next) => {
  if (error.code == 'EBADCSRFTOKEN') {
    response.status(403);
    response.send('Form tampered with, token was invalid');
  } else {
    next();
  }
});
...
app.get('/transfer', csrefMiddleware, function (request, response) {
  if (request.session.loggedin) {
    var sent = '';
    response.render('transfer', { sent, csefToken: request.csrfToken() });
  } else {
    response.redirect('/');
  }
});
...
//CSRF CODE
app.post('/transfer', csrefMiddleware, function (request, response) {
  if (request.session.loggedin) {
